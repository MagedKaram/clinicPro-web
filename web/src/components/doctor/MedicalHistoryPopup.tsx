...
import { MedicalInfoEditor } from "@/components/doctor/MedicalInfoEditor";
  ...
  return (
    ...
    >
      <div className="w-full max-w-[680px] max-h-[88vh] overflow-hidden rounded-[20px] border-2 border-doc-accent bg-doc-card shadow-doc flex flex-col">
        ...
        {/* Tab switcher */}
        <div className="flex border-b border-doc-border">
          {(["visits", "medInfo"] as Tab[]).map((tab) => (
            <button
              key={tab}
              type="button"
              onClick={() => setActiveTab(tab)}
              className={cn(
                "flex-1 py-2.5 text-[0.82rem] font-bold transition-colors cursor-pointer",
                activeTab === tab
                  ? "text-doc-accent border-b-2 border-doc-accent bg-[color-mix(in_srgb,var(--doc-accent)_6%,transparent)]"
